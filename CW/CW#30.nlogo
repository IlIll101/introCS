...
to setup
  ca
  create-cats num-cats [set shape "cat" set color brown set lives 9 setxy random-xcor random-ycor set label lives]
  create-dogs num-dogs [set shape "dog" set color grey setxy random-xcor random-ycor]
end
...
to go
  every .1 [
    ask cats [
      if any? dogs-here [set lives lives - 1]
      if lives = 0 [die]
    ]
    ask cats [wiggle set label lives]
    ask dogs [wiggle]
  ]
end

to-report alive-cats
  report count cats with [lives > 0]
end

to-report alive-dogs
  report count dogs
end
@#$#@#$#@
GRAPHICS-WINDOW
